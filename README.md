...
Используется для защиты от атак из браузеров пользователей, которые посетили страницу злоумышленника, проводящую атаку через браузер пользователя либо попытку подмена запроса.

Сервер возвращает Access-Control-Allow-Origin со списком серверов, которые имеют доступ к ресурсу для разработчиков, чтобы было понятно, какие домены могут взаимодействовать с их API.
