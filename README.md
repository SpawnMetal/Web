...
`Замыкание` — совокупность блока кода и данных того контекста, в котором тот блок порождён, т.е. это связь сущности с порождающими контекстами через цепь ЛО или ScopeChain.
...
Контролируемые и неконтролируемые элементы https://www.perplexity.ai/search/v-chem-raznitsa-mezhdu-kontrol-aANvOK__QnaBCL4sIx5cUQ

prop drilling - Передача свойств на прямую от родителя к ребенку через сложную и длинную иерархию компонентов. Избежать можно используя useContext или менеджер состояний (Flux)
...
Если в useState передаётся callback, то произойдёт ленивая инициализация состояния, можно использовать при дорогостоящих вычислениях.

Если передать за место callback саму someExpensiveComputation(props), то вычисления функции будут происходить при каждом рендере, для избежания передаём useState(someExpensiveComputation) либо оборачиваем в callback

https://ru.legacy.reactjs.org/docs/hooks-reference.html#lazy-initial-state

https://ru.react.dev/reference/react/useState#avoiding-recreating-the-initial-state

Пример использования https://codesandbox.io/p/sandbox/lazyinitialstate-y6wgsn

```jsx
// Ленивая (lazy) инициализация состояния
...
Аргумент функции обновления принято называть по первым буквам соответствующей переменной состояния.Если вы предпочитаете более подробный код, другим распространенным соглашением является повторение полного имени переменной состояния, например setEnabled(enabled => !enabled), или использование префикса, например setEnabled(prevEnabled => !prevEnabled) https://react.dev/learn/queueing-a-series-of-state-updates#naming-conventions
...
Батчинг / Пакетирование (Batching) https://react.dev/learn/queueing-a-series-of-state-updates

Резюме https://react.dev/learn/queueing-a-series-of-state-updates#recap
...
- Чтобы обновить какое-либо состояние несколько раз за одно событие, можно использовать setNumber(n => n + 1) функцию обновления.
...
Изменение DOM, которое видно пользователю, должно запускаться синхронно до следующей отрисовки, чтобы пользователь не замечал визуального несоответствия. Т. е. передавать callback не следует с async https://ru.legacy.reactjs.org/docs/hooks-reference.html#useeffect
...
Для срабатывания при обновлении зависимостей, но не первом рендеринге componentDidUpdate можно реализовать так:

```jsx
const [num, setNum] = React.useState(0)

const useDidUpdate = (callback = () => console.log('callback')) => {
...
4. `Очистка эффекта (componentWillUnmount)`: Вернуть функцию. https://ru.react.dev/reference/react/useEffect https://ru.legacy.reactjs.org/docs/hooks-reference.html#cleaning-up-an-effect

После каждого повторного рендеринга с измененными зависимостями React сначала запустит функцию очистки (если вы ее предоставили) со старыми значениями, а затем запустит вашу функцию настройки с новыми значениями. Так же, после того, как ваш компонент будет удален из DOM, React запустит вашу функцию очистки https://ru.react.dev/reference/react/useEffect
...
Вызывается после компоновки (Layout), но перед отрисовкой (Paint), чтобы не выводить то, что будет снова перерисовано, а сразу скомпоновать как нужно. Происходит перед useEffect

Вызов внутри хука setState спровоцирует рендер, даже если тот же setState уже есть в useEffect, в таком случае рендер будет вызван дважды, один из каждого эффекта.
...
Список самых популярных событий из DOM https://developer.mozilla.org/en-US/docs/Web/Events
...
Решает задачу - выяснить как наиболее эффективно обновить пользовательский интерфейс для соответствия новому дереву.
...
- Разработчик может указать какие дочерние элементы останутся стабильными между рендерами с помощью пропа key. Ключ должен быть уникальным только среди его соседей, а не глобально. https://ru.legacy.reactjs.org/docs/reconciliation.html#keys
...
Рекурсия по дочерним элементам - в списках добавлять новый элемент без key нужно в конец https://ru.legacy.reactjs.org/docs/reconciliation.html#recursing-on-children

#### React Fiber

`#Fiber`

Fiber - это механизм и базовый алгоритм для рендеринга в React 16. Основная цель - реализовать пошаговый рендеринг виртуального DOM для более быстрого рендеринга, работы с анимациями и дебагом.

React Fiber — позволяет React воспользоваться планированием. Конкретно, нам нужно иметь возможность:

Позволяет осуществлять

- Планирование — это процесс, который определяет когда работа должна быть выполнена.

1. остановить работу и вернуться к ней позже.
2. приоритизировать разные типы работы.
3. переиспользовать работу проделанную ранее.
4. отменить работу, если она больше не нужна.
...
`React Query` - библиотека для получения, кэширования, синхронизации и обновления "серверного" состояния в React-приложениях. Позволяет осуществлять API запросы.

#### React Spring

`#Spring`

https://reactdev.ru/libs/react-spring/
...
Их название всегда должно начинаться с use https://react.dev/learn/reusing-logic-with-custom-hooks#hook-names-always-start-with-use
...
https://my-js.org/docs/cheatsheet/react-table/

Позволяет создавать кастомные таблицы, но проще использовать дефолтные возможности `Material React Table` https://www.material-react-table.com/
...
Пример https://codesandbox.io/p/sandbox/react-dev-forked-h2f2nx

createPortal позволяет вам отображать некоторые дочерние элементы в другой части DOM.

Диалог можно использовать \<Dialog> из mui, он тоже размещает его в body
...
lazyпозволяет отложить загрузку кода компонента до тех пор, пока он не будет отрисован в первый раз.

В React отсутствует возможность динамического импорта из import, подобный метод позволяет это обойти
...
Пример использования https://github.com/SpawnMetal/todo

`getBy` - обычный поиск существующего элемента

`queryBy` - поиск отсутствующего элемента

`findBy` - поиск элемента после асинхронной операции

`fireEvent` - конкретный эвент

`userEvent` - вызывает цепочку эвентов, эмулируя реальные действия пользователя

Юзать await screen.findByRole вместо waitFor, потому что он под капотом у find
...
Обзор архитектуры FSD для frontend https://chat.deepseek.com/a/chat/s/e65f35e2-0bff-422a-ad2d-52732ec4a4ce
...
ЛУЧШАЯ АРХИТЕКТУРА ФРОНТЕНД? | Разбор FSD архитектуры frontend (Feature-Sliced Design) https://youtu.be/zycM3lRnbIU?si=sbgaQyD2qVSNAlH7

10 ОШИБОК НОВИЧКОВ В FSD | Feature Sliced Design методология https://youtu.be/8pcu9MwTzJo?si=iweVpDfaO7M4mEeH

Вот как выглядит иерархия слоев в FSD: app/ → pages/ → widgets/ → features/ → entities/ → shared/ → libs/

Обновленные правила зависимостей:
...
Назначение: Переиспользуемый код, не связанный с предметной областью приложения.

Что внутри: UI-кит (кнопки, инпуты, модальные окна), хелперы, утилиты, API-инстансы, константы, типы TypeScript.

Пример: shared/ui/Button, shared/lib/formatDate, shared/api/baseApi.

2. entities/ (Сущности)

Назначение: Бизнес-сущности, которые являются фундаментом для предметной области.

Что внутри: Модели данных, компоненты и логика, относящиеся к базовым бизнес-объектам. Часто соответствуют моделям на бэкенде.

Пример: entities/User (модель пользователя, компонент UserCard), entities/Product (модель товара, компонент ProductListItem).

3. features/ (Фичи)

Назначение: Пользовательские сценарии и интерактивы, которые представляют ценность для бизнеса.

Что внутри: Компоненты, которые объединяют несколько сущностей и добавляют конкретную бизнес-логику. Это "действия" пользователя.

Пример: features/AuthByEmail (форма входа), features/AddToCart (кнопка "Добавить в корзину"), features/ProductSearch (поиск по каталогу).

4. widgets/ (Виджеты)

Назначение: Композиция фич и сущностей в самостоятельные, сложные блоки интерфейса

Кросс-фичевая компоновка - объединение нескольких фич в один логический блок

Переиспользуемые секции страниц которые могут появляться на разных страницах

Что внутри:

Сложные компоненты, состоящие из нескольких фич и сущностей

Блоки с собственной компоновкой (сетки, расположение элементов)

Часто не содержат уникальной бизнес-логики, только композиционную

Отличие от Features:

Feature = конкретное пользовательское действие (авторизация, поиск, добавление в корзину)

Widget = композиционный блок, который может содержать несколько фич (сайдбар, хедер, карточка товара с действиями)

5. pages/ (Страницы)

Назначение: Композиция фич и сущностей для формирования полноценных страниц приложения.

Что внутри: Компоненты-страницы, которые часто связаны с маршрутизацией (React Router, Next.js). Здесь минимум бизнес-логики, в основном компоновка.

Пример: pages/MainPage, pages/ProductPage, pages/ProfilePage.

6. app/ (Приложение)

Назначение: Глобальная конфигурация приложения, провайдеры, стили, роутинг.

Что внутри: App.tsx, провайдеры Store (Redux), Router, Theme, глобальные стили.

Пример: app/providers, app/router, app/styles.

---

Пример с интернет-магазином с учетом Widgets:
...
├── widgets/              # Виджеты ← НОВЫЙ СЛОЙ
│   ├── header/           # Шапка сайта
│   ├── sidebar/          # Боковая панель
│   ├── productGrid/      # Сетка товаров
│   └── userProfile/      # Блок профиля пользователя
├── features/             # Фичи
│   ├── authByEmail/      # Авторизация
...
https://www.perplexity.ai/search/dlia-chego-nuzhen-dekorator-in-ggaQ81O4Swq66VPsSDJyvA

`@Injectable` Позволяет использовать класс как провайдер, методы класса становятся доступны из контроллера и к ним можно обращаться с помощью this.appService.method()
...
`#Node.js #type #module #CommonJS #ESM`
...
Решение: Добавить в package.json "type": "module", чтобы заработал import либо "target": "esnext" в tsconfig.json для обработки ES-модулей (ESM), иначе по умолчанию будет CommonJS, где импорт через require https://www.typescriptlang.org/tsconfig/#module https://habr.com/ru/companies/otus/articles/798455/
...
- Шапочка (^): ^1.2.3 — позволяет устанавливать версии от 1.2.3, но не включая, 2.0.0. Это означает, что могут быть установлены любые минорные и патч-версии, которые не нарушают основную версию.
- Тильда (~): ~1.2.3 — разрешает обновления только патчей, то есть от 1.2.3, но не включая, 1.3.0. Это более строгий подход по сравнению с шапочкой.
- Звёздочка (\*): \* — обозначает любую доступную версию пакета, что может привести к установке последней версии без ограничений.
...
Команда npx – это инструмент, который теперь поставляется с npm и позволяет запускать локальные для проекта двоичные файлы из командной строки. Например `npx ts-node reptile.ts`
...
https://proglib.io/p/express-vs-fastify-kakoy-freymvork-sleduet-ispolzovat-v-2025-godu-2025-03-31

https://habr.com/ru/articles/555668/
...
Используется для крупных проектов, так как более быстрый и надёжный, нежели express
...
Большинство решений CSS-in-JS внедряют свои стили в нижнюю часть HTML <head>, что дает Material UI приоритет над вашими пользовательскими стилями. Чтобы устранить необходимость в !important, вам нужно изменить порядок внедрения CSS. Вот демонстрация того, как это можно сделать в Material UI:
...
export default function GlobalCssPriority() {
  return <StyledEngineProvider injectFirst>{/* Your component tree. Now you can override Material UI's styles. */}</StyledEngineProvider>
}
```
...
По сути не нужен, использовать MUI.

JSS (JavaScript Style Sheets) - это инструмент для стилизации веб-приложений, который позволяет разработчикам использовать JavaScript для создания и управления стилями. Он предоставляет возможность создавать динамические стили на основе JavaScript-кода, что делает его мощным и гибким решением для управления внешним видом веб-приложений. JSS предлагает различные функции, такие как вложенные стили, использование переменных и функций, условные стили и многое другое, что делает его удобным инструментом для создания динамических и адаптивных пользовательских интерфейсов.
...
SWR — это стратегия, которая сначала возвращает данные из кэша (устаревшие), затем отправляет запрос на выборку (повторная проверка) и, наконец, предоставляет актуальные данные.

В принципе видимо лучше вместо неё использовать React Query https://habr.com/ru/articles/758360/
...
https://codelab.pro/podrobnoe-rukovodstvo-po-vite/

Vite — это невероятно быстрый инструмент сборки интерфейса, обеспечивающий работу веб-приложений нового поколения.
...
Основная идея Vite — максимальная производительность и удобство. Он создан, чтобы сделать разработку лёгкой и быстрой, убрав сложности, которые были характерны для предыдущих инструментов.

Vite как минимум позволяет:

- обновлять проект после внесения изменений, используется в связке с esbuild
- обращаться к параметрам env: const apiUrl = import.meta.env.VITE_API_URL
- совершать lazy загрузку компонентов
...
`Module Federation`, `#MF` - это плагин, созданный изначально для webpack, который позволяет разделять код между несколькими независимыми приложениями и динамически загружать его в браузере пользователя. Это может быть полезно во многих случаях, когда необходимо создать масштабируемую архитектуру фронтенд-приложений https://habr.com/ru/articles/720560/
...
Headless CMS – это системы, которые предоставляют функционал администрирования контента (админка) и генерируют API, к которому можно подключить любой клиент (веб-приложение, мобильное приложение и т.д.). Поскольку Headless CMS – это просто API, они никак не ограничивают клиент и не влияют на реализацию других сервисов бэкенда.
...
https://habr.com/ru/companies/kts/articles/837888/

Strapi - Headless CMS с открытым исходным кодом для разработчиков, которая упрощает создание API и поддерживает ваши любимые фреймворки. Настраивайте и размещайте свои проекты в облаке или на собственных серверах.
...
Пример с React: https://github.com/SpawnMetal/test_with_questions
...
observer из MobX не выполняет мемоизацию компонентов React в том же смысле, что React.memo. observer автоматически решает, когда нужно перерисовывать компонент, основываясь на зависимостях от наблюдаемых данных.
...
Redux Toolkit будет лучше чем Redux, но всё равно громоздкий, его преимущества как минимум:

- более хайповый и распространённый
- Больше возможностей
- больше инструментов для него
...
Выбор между useContext и Redux Toolkit https://chat.deepseek.com/a/chat/s/b706a211-44a1-4de6-aff0-5122bb7493fa
...
https://chat.deepseek.com/a/chat/s/c7bc1ca3-3300-4248-978c-dab13edcf5c4

Redux-Saga — это middleware для Redux, предназначенный для обработки сайд-эффектов (асинхронных операций, таких как запросы к API, таймеры и т. д.) с использованием генераторов JavaScript.

Выбор технологии в Redux Toolkit https://chat.deepseek.com/a/chat/s/0e44e6b0-d3c5-4151-a10b-3d1e74e0ebbc
...
RTK Query — мощный инструмент для извлечения и кэширования данных. Он разработан для упрощения распространенных случаев загрузки данных в веб-приложение, устраняя необходимость вручную писать логику извлечения и кэширования данных самостоятельно .

Выбор технологии в Redux Toolkit https://chat.deepseek.com/a/chat/s/0e44e6b0-d3c5-4151-a10b-3d1e74e0ebbc

React Query удобнее см [React Query](#React-Query)

#### Redux Thunk

`#Thunk`

https://chat.deepseek.com/a/chat/s/d0543b1d-87e1-4e51-a875-d33dde584748

Redux-Thunk — это middleware для Redux, который позволяет писать action creators, возвращающие не просто объект экшена, а функции (thunks). Это нужно для выполнения асинхронных операций (например, HTTP-запросов) и диспатча экшенов в разное время.
...
`ts-node` — это движок выполнения TypeScript и REPL для Node.js https://www.npmjs.com/package/ts-node

Запустить скрипт ts можно командой `npx ts-node reptile.ts` https://www.8host.com/blog/zapusk-scenariev-typescript-s-pomoshhyu-ts-node/

---

Utility Types (утилиты для работы с типами) https://www.typescriptlang.org/docs/handbook/utility-types.html

`Record<Keys, Type>` - Создает тип объекта, ключи свойств которого — Keys и значения свойств — Type. Эту утилиту можно использовать для сопоставления свойств одного типа с другим типом. https://www.typescriptlang.org/docs/handbook/utility-types.html#recordkeys-type
...
`Awaited` - для получения типа , который вернёт промис. В каком случае нужно использовать Awaited в Typescript https://www.perplexity.ai/search/kogda-ispolzovat-awaited-v-typ-Td_loD0PRFqnnZkpJuwt0Q
...
1. Расширение типов происходит посредством &, а интерфейсов посредством extends
2. Изменение типов невозможно в отличае от интерфейсов
3. extends от примитивов у интерфейсов невозможен
4. Имена интерфейсов всегда будут отображаться в сообщениях об ошибках в исходной форме, но только тогда, когда они используются по имени
...
- `unknown` - например middleware, не ясно, вернёт объект с результатом или ошибку. Так же если тип до определённого момента не извесен https://www.perplexity.ai/search/privedi-primery-ispolzovaniia-Y6NPoWBASPybixHCiB6Rcw
- `never` - например, когда функция завершится за счёт вызова throw https://www.perplexity.ai/search/privedi-primery-ispolzovaniia-Y6NPoWBASPybixHCiB6Rcw
- `Function` - функция
...
Индексы (Indexes) https://habr.com/ru/companies/ruvds/articles/724066/ https://www.digitalocean.com/community/tutorials/how-to-use-indexes-in-mysql
...
Основы Redis, его возможности для кэширования, работы с различными структурами данных и механизмами публикации/подписки. Примеры. https://www.perplexity.ai/search/opishi-osnovy-redis-ego-vozmoz-QdZkYDZjSw6WjXWrPtLurw
...
Repaint / Reflow. `Repaint` - В случае изменения стилей элемента, не влияющих на его размеры и положение на странице (например, background-color, border-color, visibility), браузер просто отрисовывает его заново, с учётом нового стиля — происходит repaint (или restyle). `Reflow` - Если же изменения затрагивают содержимое, структуру документа, положение элементов — происходит reflow (или relayout). Причинами таких изменений обычно являются:
...
`#Render #CRP #Критические #этапы #рендеринга`
...
Определяет размеры и позицию каждого элемента на странице. Как только компоновка определена - пиксели отрисовываются на экране. Так же возникает при изменении размеров окна в браузере.
...
Проверьте зависимости проекта. Избавьтесь от всего ненужного. см [Tree shaking](#Tree-shaking)
...
Google I/O 2012 - Breaking the JavaScript Speed Limit with V8 https://youtu.be/UJPdhx5zTaw?si=RyisHG_uYRQOreFg https://habr.com/ru/articles/154537/

Писать мономорфные функции - если функция запускается с параметрами одного типа, не нужно запускать её с параметрами других типов впоследствие.

Использовать 31 битовые SMIs числа, не использовать в одном массиве их с double либо делать это при инициализации массива.

Функции должны быть линейны.

Не попадаться на деоптимизацию, null, undefined и true ведут себя как объекты, не являясь ими по факту.
...
Так же см [React Profiler](#React-Profiler)
...
Lighthouse https://developers.google.com/web/tools/lighthouse https://habr.com/ru/companies/htmlacademy/articles/585866/

Core Web Vitals https://habr.com/ru/articles/568280/
...
HTTP (Hypertext Transfer Protocol - гипертекстовый транспортный протокол) https://developer.mozilla.org/ru/docs/Glossary/HTTP https://ru.wikipedia.org/wiki/HTTP

Протокол передачи гипертекста (HTTP) является базовым сетевым протоколом, который позволяет передавать гипермедиа документы в Web, обычно между браузером и сервером, таким образом, чтобы люди могли их читать.
...
Отличие версий HTTP. Основная - скорость. Во 2 версии пакетная отправка сообщений без ожиданий ответа, в 3 версии их оптимизация, бинарный формат со сжатием, новый протокол QUIC https://habr.com/ru/articles/739166/ https://developer.mozilla.org/ru/docs/Web/HTTP/Guides/Connection_management_in_HTTP_1.x https://developer.mozilla.org/ru/docs/Glossary/HTTP_2 https://developer.mozilla.org/en-US/docs/Glossary/HTTP_3 https://ru.wikipedia.org/wiki/HTTP/3
...
`1XX` - Информирование о процессе передачи. В HTTP/1.0 — сообщения с такими кодами должны игнорироваться. В HTTP/1.1 — клиент должен быть готов принять этот класс сообщений как обычный ответ, но ничего отправлять серверу не нужно. Сами сообщения от сервера содержат только стартовую строку ответа и, возможно, несколько полей заголовка. Прокси-серверы подобные сообщения должны отправлять дальше от сервера к клиенту.

`2XX` - Успешное выполнение.

`3XX` - Указывают клиенту, что для успешного выполнения операции необходимо сделать другой запрос (как правило по другому URI). Из данного класса пять кодов 301, 302, 303, 305 и 307 относятся непосредственно к перенаправлениям (редирект). Адрес, по которому клиенту следует произвести запрос, сервер указывает в заголовке Location. При этом допускается использование фрагментов в целевом URI.
...
- `GET` запрашивает представление ресурса. Запросы с использованием этого метода могут только извлекать данные. Является `идемпотентным`, `безопасным`, `кешируемым`.
- `HEAD` запрашивает ресурс так же, как и метод GET, но без тела ответа. Является `идемпотентным`, `безопасным`, `кешируемым`.
- `POST` используется для отправки сущностей к определённому ресурсу. Часто вызывает изменение состояния или какие-то побочные эффекты на сервере.
- `PUT` заменяет все текущие представления ресурса данными запроса. Является `идемпотентным`.
- `DELETE` удаляет указанный ресурс. Является `идемпотентным`.
- `CONNECT` устанавливает "туннель" к серверу, определённому по ресурсу.
- `OPTIONS` используется для описания параметров соединения с ресурсом. Официально называется `preflight request`. Является `идемпотентным` и `безопасным`. Метод OPTIONS отправляется из браузера автоматически, только если выполняется кросс-доменный запрос [CORS](#CORS), он считается "сложным".
- `TRACE` выполняет вызов возвращаемого тестового сообщения с ресурса.
- `PATCH` используется для частичного изменения ресурса. `Не обязательно идемпотентен`, хотя может быть.

`Идемпотентный метод` - Единичный и множественные вызовы этого метода, с идентичным набором данных, будут иметь тот же результат выполнения (без сторонних эффектов), в случае с POST, множественный вызов с идентичным набором данных может повлечь за собой сторонние эффекты https://developer.mozilla.org/ru/docs/Glossary/Idempotent

Метод HTTP является `безопасным`, если он не изменяет состояние сервера. Другими словами, безопасный метод производит операции "только для чтения". Следующие методы HTTP являются безопасными: GET, HEAD и OPTIONS. Все безопасные методы являются также идемпотентными, но не все идемпотентные методы являются безопасными. Например, методы PUT и DELETE идемпотентные, но не безопасные https://developer.mozilla.org/ru/docs/Glossary/Safe

Кешируемые методы https://developer.mozilla.org/ru/docs/Glossary/Cacheable

HTTP-кеширование (`Cache-Control`) https://developer.mozilla.org/ru/docs/Web/HTTP/Guides/Caching

#### HTTPS

`#HTTPS`

Браузер проверяет в сертификате полученном с сервера: доменное имя, срок действия и кем был выдан. В браузере содержатся встроенные списки доверенных центров сертификации, чтобы проверить подлинность сертификатов. Отображается соответствующий значок/уведомление, если сертификат соответствует требованиям. Затем данные шифруются с помощью сертификата при обмене информацией.
...
`"workbench.editor.wrapTabs": true` - Перенос открытых файлов на новые строки
...
Документация https://docs.cursor.com/welcome

IDE на базе VSCode + AI ассистент с выбором из списка
...
https://expressjs.com/ru/guide/writing-middleware.html#:~:text=Функции%20промежуточной%20обработки%20(middleware)%20-,как%20правило%2C%20обозначается%20переменной%20next%20
...
`Аутентификация` - осуществляется при вводе логина с паролем; убеждаемся, что пользователь тот, за кого себя выдаёт

`Авторизация` - доступ у пользователя к механизмам приложения в зависимости от прав
...
CORS расшифровывается как Cross-Origin Resource Sharing. Это механизм браузера, который позволяет определить список ресурсов, к которым страница может получить доступ. Он нужен для обеспечения безопасности и защиты пользователей от злоумышленников при использовании HTTP/HTTPS-протокола.

CORS работает только в браузере https://chat.deepseek.com/a/chat/s/5eb0b9f8-9ce6-41a1-96c1-31b012cd3412

Используется для защиты от атак из браузеров пользователей, которые посетили страницу злоумышленника, проводящую атаку через браузер пользователя либо попытку подмены запроса.

Заголовок Access-Control-Allow-Origin может содержать только один источник или символ \*, который разрешает доступ всем доменам. Несколько разных адресов через запятую не поддерживаются.

Сама настройка CORS проводится на сервере, в NestJS включаются функцией app.enableCors()
...
Атаку, при которой хакер пытается выполнить авторизованный запрос на вашем сайте, используя доступ, предоставленный пользователем, называют CSRF-атакой (cross-site request forgery – межсетевая подделка запроса).

Хакер пытается выполнить авторизованный запрос через доступ пользователя.

CSRF расшифровывается как cross-site request forgery или межсайтовая подделка запроса. Это вид атаки, когда злоумышленник получает доступ к данным из браузера другого человека и выполняет действия от его лица. Например, изменяет пароль от личного кабинета на сайте, где уже авторизован этот пользователь.

Способы защиты https://habr.com/ru/companies/banki/articles/759058/
...
https://sky.pro/wiki/javascript/ponimanie-vanilla-js-eto-freymvork-ili-syroy-java-script/

VanillaJS (Ванильный JS) – это чистый JavaScript, в который не входят никакие библиотеки или фреймворки. Он превосходит по быстродействию и поддерживается всеми браузерами без модификаций.

### Асинхронность, многопоточность и параллелизм

`#Асинхронность #Многопоточность #Параллелизм`

https://habr.com/ru/articles/337528/

Так же см [JavaScript](#JavaScript)

Асинхронность - возможность приостановки выполнения одной задачи с переходом к другой или обратно

Многопоточность - выполнение в разных потоках разных или тех же задач

Параллелизм - в неё входит Асинхронность и Многопоточность, как технологии, а Параллелизм просто термин.

### Side effect

`#Side #effect #сайд #эффект`

Код, который делает функцию грязной. См [Чистая функция](#Чистая-функция)
...
Поток документа - Это принцип организации элементов на странице при отсутствии стилей https://doka.guide/html/flow/
...
Пример https://codesandbox.io/p/sandbox/css-specificity-74m887
...
https://blog.skillfactory.ru/rabbitmq-chto-eto-takoe-i-kak-rabotaet/
